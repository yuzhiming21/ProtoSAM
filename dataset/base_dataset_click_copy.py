...
class BaseVolumeDataset(Dataset):
    def __init__(
        ...
        # 多模态标记符号
        self.modal_idx = 0
        self.spatial_index = (0, 1, 2)  # 新增默认值
    ...
    def __getitem__(self, idx):
        ...
        # 加载数据（处理可能的字典格式）
        loaded_img = torch.load(img_path, weights_only=False)
        loaded_seg = torch.load(label_path, weights_only=False)
        
        # 提取实际数据（支持直接张量或字典格式）
        img = loaded_img["img"] if isinstance(loaded_img, dict) else loaded_img
        seg = loaded_seg["GT"] if isinstance(loaded_seg, dict) else loaded_seg
        ...
        if img.ndim == 4:  # (C, D, H, W)
            ...
            # 如果是SAM模型需要3通道
                img = img[:3]  # 取前3个通道
                if img.shape[0] == 1:  # 如果只有1个通道
                    img = np.repeat(img, 3, axis=0)  # 复制为3通道
            else:
                # 普通3D医学图像取第一个通道
                img = img[0]  # -> (D, H, W)
        ...
        # 确保最终是3D
        if img.ndim != 3:
            raise ValueError(f"最终图像必须是3D，当前形状: {img.shape}")

        # 同样的处理应用于标签数据
        if seg.ndim == 5:
            seg = seg[0, 0]  # 假设标签是 (1, 1, D, H, W)
        elif seg.ndim == 4:
            seg = seg[0]  # 假设标签是 (1, D, H, W)
        ...
        # 转换为PyTorch张量用于后续插值
        img_tensor = torch.from_numpy(img).unsqueeze(0)  # 添加batch和channel维度
        seg_tensor = torch.from_numpy(seg).unsqueeze(0).unsqueeze(0)
        ...
        if (np.max(img_spacing) / np.min(img_spacing) > 8) or (
            ...
            img = (
                F.interpolate(
                    input=img_tensor.unsqueeze(0).permute(0, 2, 1, 3, 4).contiguous(),  # 对 img_tensor 增加了一个额外的维度（unsqueeze(0)），然后通过 permute(0, 2, 1, 3, 4) 调整维度顺序
            ...
            img = (
                F.interpolate(
                    ...
                    scale_factor=tuple(     # scale_factor 是在三个维度上进行计算的，保证图像和标签的空间分辨率都与目标分辨率一致
        ...
        if self.split == "train":   # 训练集返回增强后的图像（img_aug）、标签（seg_aug）和图像的空间分辨率（img_spacing）
            return img_aug, seg_aug, np.array(img_spacing)#, img_path
        else:       # 验证集和测试集额外返回图像的路径（img_path），便于进一步追踪或保存结果
            return img_aug, seg_aug, np.array(img_spacing), img_path    #.split("/")[-1]
                ...
                transforms.extend(
                    [
                        ...
                        RandZoomd(
                            keys=["image", "label"],
                            prob=0.3,
                            min_zoom=[1, 0.9, 0.9],     # 缩放比例范围；此处表示在第一个维度上不变，后两个维度进行最小0.9的缩放
                            max_zoom=[1, 1.1, 1.1],
